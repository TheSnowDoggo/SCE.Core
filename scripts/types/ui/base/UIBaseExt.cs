namespace SCE
{
    public class UIBaseExt : UIBase
    {
        ...
        public UIBaseExt(string name, int width, int height, Color? bgColor = null)
            : base(name)
        {
            _dpMap = bgColor is Color color ? new(width, height, color) : new(width, height);
        }

        public UIBaseExt(string name, Vector2Int dimensions, Color? bgColor = null)
            : this(name, dimensions.X, dimensions.Y, bgColor)
        {
        }

        public UIBaseExt(int width, int height, Color? bgColor = null)
            : this(string.Empty, width, height, bgColor)
        {
        }

        public UIBaseExt(Vector2Int dimensions, Color? bgColor = null)
